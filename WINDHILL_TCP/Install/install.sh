...
# wcp 启动脚本
wcp_start_script_path=${this_file_path}
wcp_start_script_name=wcp_start.sh
# 版本信息
version_ini_path=${this_file_path}
version_ini_name=info.ini

# TCP 安装路径, 更新脚本安装路径, wcp 启动脚本安装路径， 版本信息安装路径
tcp_install_path=/opt/DL_Manager
...
function InstallManagerPro() {
    ...
    # 安装目录不存在
    if [[ ! -d ${tcp_install_path} ]]; then
        printf "mkdir install path: %s\n" "${tcp_install_path}"
        mkdir -p "${tcp_install_path}" || return 1
    fi
...
function InstallManager() {
	...
	# tcp 管理程序
	cp "${pro_tcp_exe}" ${tcp_install_path} || return 3

	local exe_real_path="${tcp_install_path}"/WINDHILL_TCP
...
function InstallManagerPost() {
	...
	# 复制更新脚本
	cp "${update_script_path}/${update_script_name}" "${tcp_install_path}" || return 1
	chmod +x "${tcp_install_path}/${update_script_name}" || return 1

	# 复制 wcp 启动脚本
	cp "${wcp_start_script_path}/${wcp_start_script_name}" "${tcp_install_path}" || return 1
	chmod +x "${tcp_install_path}/${wcp_start_script_name}" || return 1

	# 复制版本信息
	cp "${version_ini_path}/${version_ini_name}" "${tcp_install_path}" || return 1
	chmod +r "${tcp_install_path}/${version_ini_name}" || return 1
