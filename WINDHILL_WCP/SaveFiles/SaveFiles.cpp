...
int save_files(uint16_t FileNO, uint32_t Length) {
    ...
    std::string absoulte_data_dir = DATA_DIR + g_File_Info.strLocalDirName;
    char old_path[256] = {0};
    getcwd(old_path, sizeof(old_path));
    DIR *pDir = opendir(absoulte_data_dir.c_str());
    if (NULL == pDir) {
        XLOG_DEBUG("Directory {} doesn'e exist.", absoulte_data_dir.c_str());
        int ret = 0;
        if ( (ret = mkdir(absoulte_data_dir.c_str(), 0775)) == 0) {
            XLOG_INFO("Make directory {} to save data.", absoulte_data_dir.c_str());
        } else {
    ...
    XLOG_DEBUG("The time taken to save data file {}: {:.3f} ms.", file_name, 1.0 * (end - start) / 1000 );
#endif
...
int save_last_file() {
    ...
    if (g_File_Info.bIsActive == false) return 0;
...
void *save_msg_thread(void *arg) {
    ...
    // 总大小 = 8 *（信号个数 + 时间戳个数）
    int single_bytes = 8 * (1 + singals_count);
    // 表示是否出错
    int signal_error_flag = 0;
    ...
    while(runSaveThread) {
        ...
        // 缓冲区写满，创建线程保存到文件
        if (BUFFER_LENGTH + single_bytes > MSG_BUFFER_SIZE) {
            while (runSaveFileThread) {
                usleep(1000);
        ...
        
        {
            ...
            memcpy(buffer_ptr + BUFFER_LENGTH, &time_stamp, sizeof(time_stamp));
            ...
            while (ItrMsg != g_stu_SaveMessage.msg_struct.msg_list.end() && (signal_index < singals_count)) {
                ...
                memcpy(buffer_ptr + BUFFER_LENGTH + sizeof(time_stamp) + sizeof(dval) * signal_index++, &dval, sizeof(dval));

                ItrMsg++;
            }
            if (signal_index < singals_count) {
                XLOG_ERROR("signal count ERROR, timestamp: {:X}(h).", time_stamp);
                signal_error_flag = 1;
            }
        }
        // 若有错误发生，则不改变写入索引
        if (0 == signal_error_flag) {
            BUFFER_LENGTH += single_bytes;
        } else {
            signal_error_flag = 0;
        }
...
int save_init() {
    ...
    // 2022.06.24
    // 创建数据文件夹
    std::string data_absoulte_dir = DATA_DIR + g_File_Info.strLocalDirName;
    if (0 != access(data_absoulte_dir.c_str(), F_OK)) {
        if (-1 == mkdir(data_absoulte_dir.c_str(), 0775) ) {
            XLOG_DEBUG("Make data directory ERROR: {}.", strerror(errno));
            return -1;
        }
    }
