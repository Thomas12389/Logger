...
int map_sigout2sigin(rapidxml::xml_node<> *pMsgOutNodes, int nNumMessages) {
    ...
    for (int index = 0; index < nNumMessages; index++) {
        ...
        // 若配置文件中内置信号无 Channel 节点，如 GPS 或者 Internal 等, 
        if (NULL == pNode) {
            pMsgNode = pMsgNode->next_sibling("Message");
            continue;
        }
        ...
        // 若配置文件中内置信号有 Channel 节点（此时 Channel 需为空）
...
// printf("OBD -- RequestID: 0x%02X, ResponseID: 0x%02X, PID: %d, <%d>th signal.\n", ItrOBD->first, ItrRSP->first, pPID[idxPID].nPID, idxSig);
                            pSig[idxSig].bIsSend = 1;
                            pSig[idxSig].strOutName = pMsgNode->first_node("MessageName")->value();
                            // OBD 信号值初始化
                            g_stu_OutMessage.msg_struct.msg_list.push_back({pSig[idxSig].strOutName, SIGNAL_NAN, 
