...
int parse_xml_storage(rapidxml::xml_node<> *pStorageNode, File_Save& File_Info){
    ...
    rapidxml::xml_node<> *pNode = pStorageNode->first_node("active");
    if (NULL == pNode) {
        return -1;
    }
    char *pActive = pNode->value();
    if (0 == strncmp("true", pActive, strlen("true"))) {
        File_Info.bIsActive = true;
    } else {
        File_Info.bIsActive = false;
        return 0;
    }
    pNode = pStorageNode->first_node("FileName");
    File_Info.strConfigFileName = pNode->value();
...
// printf("XCP -- 0x%02X <%ld>th DAQList <%d>th ODT <%d>th Ele\n", (*ItrDAQList).nCANID, std::distance((ItrXCPRcv->second).begin(),ItrDAQList), nIdxODT, nIdxEle);
            ...
            OBD_MAP_MsgID_Mode01_RMsgPkg::iterator ItrOBD = (Itr->second).CAN.OBDII.mapOBD.begin();

            for (; ItrOBD != (Itr->second).CAN.OBDII.mapOBD.end(); ItrOBD++) {
                std::map<uint32_t, OBD_Mode01RecvPackage*>::iterator ItrRSP = (ItrOBD->second).begin();

                for (; ItrRSP != (ItrOBD->second).end(); ItrRSP++) {

                    OBD_Mode01PID *pPID = (ItrRSP->second)->pOBD_PID;
                    for (uint8_t idxPID = 0; idxPID < (ItrRSP->second)->nNumPID; idxPID++) {

                        OBD_Mode01Signal *pSig = pPID[idxPID].pOBD_Mode01Signal;
                        for (uint8_t idxSig = 0; idxSig < pPID[idxPID].nNumSignal; idxSig++) {

                            if (pName != pSig[idxSig].strOBDSignalName) continue;
// printf("OBD -- RequestID: 0x%02X, ResponseID: 0x%02X, PID: %d, <%d>th signal.\n", ItrOBD->first, ItrRSP->first, pPID[idxPID].nPID, idxSig);
                            pSig[idxSig].bIsSave = 1;
                            pSig[idxSig].strSaveName = pMsgNode->first_node("MessageName")->value();
                            // OBD 信号值初始化
                            g_stu_SaveMessage.msg_struct.msg_list.push_back({pSig[idxSig].strSaveName, SIGNAL_NAN, 
                                                    pSig[idxSig].strOBDSignalUnit, pSig[idxSig].strOBDSignalFormat});
                        }
                    }
                }
            }
    ...
    for (; ItrList != g_stu_SaveMessage.msg_struct.msg_list.end(); ItrList++) {
        printf("%d  name : %s, val = %lf, uint : %s\n", idx, (*ItrList).strName.c_str(), (*ItrList).dPhyVal, (*ItrList).strPhyUnit.c_str());
    }
    printf("\n");
#endif
