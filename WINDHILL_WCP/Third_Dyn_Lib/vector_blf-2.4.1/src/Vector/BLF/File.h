...
class VECTOR_BLF_EXPORT File final {
  public:
    ...
    // debug add: 2022.6.14
    /**
     * Get timestamp when file is opend
     */
    std::int64_t getOpenTimestamp();

    /**
     * Get timestamp(ns) now
     */
    std::int64_t getNowTimestamp();

    /**
     * Write object to file, and update lastObjectTime.
     *
     ...
     * @param[in] ts  absoulte timestamp [ns], the lastest object timestamp
     */
    void write(ObjectHeaderBase * ohb, int64_t ts);
    //
  ...
  private:
    // debug add: 2022.6.14
    /**
     * timestamp(ns) open file
     */
    std::int64_t m_openTimestamp {};
    //
    ...
    // debug add: 2022.6.14
    /**
     * Convert timestamp(ns) to Vector::BLF::SYSTEMTIME
     */
    SYSTEMTIME timestamp2SystemTime(std::int64_t ns_timestamp);
    //
