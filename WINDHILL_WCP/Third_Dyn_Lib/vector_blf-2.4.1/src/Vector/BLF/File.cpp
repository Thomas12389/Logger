...
// debug add: 2022.6.14
File::File(uint64_t baseTime) : m_openTimestamp(baseTime) {
    /* set performance/memory values */
    m_readWriteQueue.setBufferSize(0x1000000);
    m_uncompressedFile.setBufferSize(m_uncompressedFile.defaultLogContainerSize());
}

File::File():
    m_openTimestamp(std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now().time_since_epoch()).count()) {
    /* set performance/memory values */
    m_readWriteQueue.setBufferSize(0x1000000);
    m_uncompressedFile.setBufferSize(m_uncompressedFile.defaultLogContainerSize());
}
...
// debug add: 2022.6.14
std::int64_t File::getOpenTimestamp() {
    return m_openTimestamp;
}

std::int64_t File::getNowTimestamp() {
    return std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
}

SYSTEMTIME File::timestamp2SystemTime(std::int64_t ns_timestamp) {
    auto tp = std::chrono::time_point<std::chrono::system_clock, std::chrono::nanoseconds>((std::chrono::nanoseconds)ns_timestamp);
    auto tt = std::chrono::system_clock::to_time_t(tp);
    std::tm *now = std::gmtime(&tt); // 转换为本地时间使用 std::localtime(&tt)， 格林威治时间使用 std::gmtime(&tt)
    return {static_cast<uint16_t>(now->tm_year + 1900), static_cast<uint16_t>(now->tm_mon + 1), static_cast<uint16_t>(now->tm_wday), 
            static_cast<uint16_t>(now->tm_mday), static_cast<uint16_t>(now->tm_hour), static_cast<uint16_t>(now->tm_min), 
            static_cast<uint16_t>(now->tm_sec), static_cast<uint16_t>((ns_timestamp / 1000) % 1000)};
}

void File::write(ObjectHeaderBase * ohb, int64_t ts) {
    // fileStatistics.lastObjectTime = timestamp2SystemTime(getNowTimestamp());
    fileStatistics.lastObjectTime = timestamp2SystemTime(ts);
    write(ohb);
}
// 
...
void File::open(const char * filename, const std::ios_base::openmode mode) {
    ...
    if (mode & std::ios_base::in) {
        ...
        /* 设置线程名称 */
        pthread_setname_np(m_uncompressedFileThread.native_handle(), "m_uncompress_r");
        pthread_setname_np(m_compressedFileThread.native_handle(), "m_compress_r");
    } else

        /* write */
        if (mode & std::ios_base::out) {
            // debug add: 2022.06.14
            fileStatistics.measurementStartTime = timestamp2SystemTime(getOpenTimestamp());
            //
            ...
            /* 设置线程名称 */
            pthread_setname_np(m_uncompressedFileThread.native_handle(), "m_uncompress_w");
            pthread_setname_np(m_compressedFileThread.native_handle(), "m_compress_w");
        }
}
