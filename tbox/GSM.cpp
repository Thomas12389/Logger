...
int gsmStart(void)
{
	...
	XLOG_DEBUG("TBox--> GSM Moudle is active {}.", iIsActive ? "OK" : "FAIL");
	...
	while( runGSMHandlerEvents == TRUE){
		...
		switch ( owEvents->gsmEventType){
			case GSM_NO_SIGNAL:
 			...
 			case GSM_FAILURE:
				printf( "----------- GSM ERROR ------------");
				XLOG_DEBUG("GSM ERROR.");
				gsmError = TRUE;
				break;
			case GSM_COVERAGE:
				printf( "GSM EVENT--> GSM COVERAGE INFO: %d\n", owEvents->evBuffer[0]);
				XLOG_DEBUG("GSM EVENT--> GSM COVERAGE INFO: {:d}", owEvents->evBuffer[0]);
				break;
			case GSM_HIGHER_TEMP:
				printf( "GSM EVENT--> GSM HIGHER TEMP. Recommended GSM switch off\n");
				XLOG_DEBUG("GSM EVENT--> GSM HIGHER TEMP. Recommended GSM switch off.");
				// 蜂鸣器
				break;
			case GSM_GPRS_COVERAGE:
				printf( "GSM EVENT--> GPRS COVERAGE INFO: %d\n", owEvents->evBuffer[0]);
				XLOG_DEBUG("GSM EVENT--> GPRS COVERAGE INFO: {:d}", owEvents->evBuffer[0]);
				break;
			case GSM_BOARD_TEMP:
				...
				break;
			default:
				printf( "TBox--> Signal Event not found ...%d \n", owEvents->gsmEventType);
		}
	}
	return NULL;
}
