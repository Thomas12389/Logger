...
old_files_deal_target_sh=/home/debian/deal_old_files.sh
# 公共文件路径
	...
	if Start_BT_WiFi 1; then
    	...
    	if ip link set mlan0 up; then
			...
			if [ ! -e "${WLAN_script_path}" ]; then
				mkdir -p ${WLAN_script_path}
			fi
			
			cp "${this_file_path}"/files/WLAN/*.sh ${WLAN_script_path}

			cd "${WLAN_script_path}" || return 1
...
function CopyLibFiles() {
	...
	local cur_dir
	local lib_files_list="libssh2.so.1.0.1 libcrypto.so.1.0.0 libssl.so.1.0.0 libmosquitto.so.1 libftp.so.4.0"
...
function InstallLogger() {
	...
	# wcp 采集程序
	# if [ ! -x "${pro_wcp_exe}" ]; then
	# 	echo "file \"${pro_wcp_exe}\" is not executable"
	# 	return 3
	# fi
	cp "${pro_wcp_exe}" ${install_path} || return 1

	local exe_real_path="${install_path}"/WINDHILL_WCP
